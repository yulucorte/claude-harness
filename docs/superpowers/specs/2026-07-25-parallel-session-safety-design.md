...
- `classify()` reconoce además los verbos que reescriben el árbol de trabajo: `checkout`, `switch`, `restore`, y `reset` **sólo** cuando lleva `--hard` (un `reset --soft`/`--mixed` no toca los archivos en disco).
  ...
  **Ampliado durante la implementación** (revisión final de la rama, 2026-07-25): el conjunto anterior dejaba fuera precisamente los comandos que git **no** protege por su cuenta. Git se niega a un `checkout`/`merge` que pisaría archivos modificados; `git clean -f` y `git stash` destruyen sin preguntar. Comprobado en repo de prueba: `git clean -fd` borró el archivo sin seguimiento del peer, y `git stash -u` revirtió su edición sin commitear y borró su archivo sin seguimiento. Además, un agente al que se le acaba de denegar un `checkout` recurre con toda naturalidad a `git stash`. Se añaden por tanto al mismo camino graduado: `pull` (fetch + merge/rebase sobre el árbol), `clean` cuando lleva `-f`/`-ff`/`--force` (agrupadas incluidas: `-fd`, `-fdx`), y las formas destructivas de `stash` (`git stash`, `stash push`, `stash save`, `stash -u`, y `stash pop`/`apply` sin referencia `stash@{n}`). `stash list`/`show` siguen sin vigilarse y una referencia `stash@{n}` explícita conserva su trato anterior; la regla 3 (stash compartido por todo el repo, incluidos los worktrees) sigue aplicando igual cuando **ningún** peer comparte la raíz del worktree.
...
- **El clasificador de comandos lee tokens, no ejecuta nada.** `bash -c "git checkout main"`, `sh -c ...`, un alias, o cualquier operación de git dentro de un script `.sh` son invisibles para él: el hook recibe el comando como texto y sólo reconoce un `git <verbo>` literal. Como este cambio elimina el aislamiento automático, la denegación en `PreToolUse` pasa a ser la **única** barrera contra la clase catastrófica; sorteado el clasificador, no queda red debajo. Se acepta a sabiendas: interpretar shell de verdad (expandir alias, seguir scripts) no es viable en un hook y produciría más falsos positivos que protección.
- **Las escrituras hechas vía Bash nunca llegan a `files`.** `sed -i`, una redirección con heredoc, `cp`, `mv` o un script modifican archivos sin pasar por `Write`/`Edit`/`NotebookEdit`, que son las únicas herramientas que el heartbeat registra. El aviso por archivo compartido **subreporta por construcción**: su ausencia no significa que nadie más haya tocado el archivo.
- **`git revert`, `git am`, `git apply` y `git rm -r` no se vigilan.** Los cuatro pueden modificar el árbol ajeno. Se dejan fuera porque su uso normal es deliberado y acotado (un revert concreto, un parche concreto), y denegarlos añadiría fricción constante sin cubrir la clase catastrófica, que es la reescritura *masiva* del árbol. Decisión revisable, no olvido: `tests/test-guardian-verb-coverage.sh` fija su silencio como comportamiento esperado, de modo que cambiarlo exija cambiar el test.
