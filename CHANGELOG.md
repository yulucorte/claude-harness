...
Un solo arreglo, de una línea, que decide si el resto del plugin existe o no
para el agente.

### Corregido — nada de lo que slate inyectaba al arrancar llegaba al agente (BUG-003)

`session-start.sh` emitía el formato **plano** `{"additionalContext": ...}`.
Claude Code lo ejecuta sin error pero lo **descarta en silencio** cuando varios
plugins cablean `SessionStart` a la vez: solo sobrevive el formato envuelto
`{"hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": ...}}`.

Y la competencia es el caso **normal**, no el raro: Superpowers 6.3.0 cablea
`SessionStart` con matcher `startup|clear|compact`, y `plugin.json` define slate
como *"Lightweight companion to Superpowers"*. Es decir, el modo de fallo estaba
activo en la configuración para la que slate fue diseñado.

Lo que se perdía en cada arranque: índice de in-progress, estado en vuelo,
historial reciente, bugs abiertos, conteo de ideas, versión del plugin,
contador de `history.md` y el truncado de `current.md`. **Las cinco capacidades
que 1.9.0 añadió dentro de este hook nunca llegaron a uso real.**

Es exactamente el test que 1.8.0 aplicó para borrar `pre-compact.sh`: corre, no
falla, y nadie lee lo que escribe. La diferencia es que aquí el hook sí tenía
lector — lo que faltaba era el sobre correcto. `session-lock.sh:156` ya emitía
envuelto desde FEAT-001, y el bug quedó **documentado** en las notas de
`done.md` el 2026-07-19 sin corregirse aquí por decisión de alcance: 26 días
abierto, invisible porque el síntoma es silencio.

**Medición que lo confirmó** (no simulada, mismo método que FEAT-001): sesión
real con slate + Superpowers, leyendo el transcript `.jsonl`. El bloque de
Superpowers (envuelto) aparece como `type=attachment`; el de slate (plano) no
aparece jamás. El hook sí corre y sí produce 1189 bytes — falla la entrega, no
la ejecución.

### Añadido

- `tests/test-session-start-wrapped-output.sh` — 4 aserciones, incluida una que
  falla si el formato plano reaparece en el fuente.

### Nota sobre los tests preexistentes

`test-session-start-current-truncate.sh` y `test-session-start-version-history-count.sh`
decodificaban la clave plana y estaban **en verde contra un formato que nunca
llegaba a producción**. Actualizados al envelope. Sirve de recordatorio: un test
que consume la salida de un hook debe leerla como la lee Claude Code, no como la
escribe el script.
